...
void the_program(uint8_t core_id, unsigned int log_interval, char* tcp_db_source_name, int gre_offset) {
    ...
    printf("Zero-copy TCP ECN Analyzer child proc started on core %d!\n", core_id);
...
void sigproc_child(int sig)
{
    ...
    fprintf(stderr, "PF_RING zero-copy TCP ECN Analyzer child process shut down\n");
    exit(0);
}
...
void sigproc_parent(int sig)
{
    ...
    fprintf(stderr, "PF_RING TCP ECN Analyzer shutting down...\n");
    ...
    fprintf(stderr, "PF_RING TCP ECN Analyzer done shutting down!\n");
    exit(0);
}
